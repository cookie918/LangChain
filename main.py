...
# # 修改成自己的配置！！！
class LangChainCFG:
    llm_model_name = 'D:\\Chinese-LangChain\\THUDM_chatglm-6b'  # 本地模型文件 or huggingface远程仓库
    embedding_model_name = 'D:\\Chinese-LangChain\\text2vec-large-chinese'  # 检索模型文件 or huggingface远程仓库
    vector_store_path = './source'
    docs_path = './docs'
    kg_vector_stores = {
        '中文维基百科': './source/zh_wikipedia',
        '大规模金融研报': './source/financial_research_reports',
        '初始化': './source',
    }  # 可以替换成自己的知识库，如果没有需要设置为None
    # kg_vector_stores=None
    patterns = ['模型问答',     # 使用transformers加载模型预测
                '知识库问答']   # 
    n_gpus=1
...
with gr.Blocks(css=customCSS, theme=small_and_beautiful_theme) as demo:
    ...
    with gr.Row():
        ...
        with gr.Column(scale=4):
            with gr.Row():
                chatbot = gr.Chatbot(label='Chinese-LangChain')
            with gr.Row():
